...
namespace MVCOnlineTicariOtomasyon.Controllers
{
    public class istatistikController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var deger13=c.Uruns.Where(u=>u.UrunID == c.satisHareketleris.GroupBy(x=>x.Urunid).OrderByDescending(z=>z.Count()).Select(y=>y.Key).FirstOrDefault()).Select(k=>k.UrunAd).FirstOrDefault();
            ViewBag.d13 = deger13;

            var deger14 = c.satisHareketleris.Sum(x => x.ToplamTutar).ToString();             
            ViewBag.d14 = deger14;

            DateTime bugun = DateTime.Today;
            var deger15=c.satisHareketleris.Count(x=>x.Tarih ==bugun).ToString();
            ViewBag.d15 = deger15;

            var deger16 = c.satisHareketleris.Where(x => x.Tarih == bugun).Sum(y => (decimal?)y.ToplamTutar).ToString();
            ViewBag.d16 = deger16;
            return View();
        }
